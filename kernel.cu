      ...
      if (row >= id * (m / 2) && row < (id + 1) * (m / 2)) {
        ...
        for (int i = 0; i < (int)(ceil((float)n / TILE_DEPTH)); i++) {
          ...
          for (int j = 0; j < TILE_DEPTH; j++) {
            dist[py][px] += (rowVector[py][j] - colVector[j][px]) *
                            (rowVector[py][j] - colVector[j][px]);
          }
          __syncthreads();
        }
...
extern "C" void beforeStart(const char *hostname) {
  float *dA;
...
void launch(int id, int m, int n, int k, int *V, int *out) {
  ...
  knn<<<(m / 2), threadNum, 2 * ptrNumInSMEM * sizeof(int)>>>(id, m, k, d_V, D,
                                                              d_out);

  // copy result back to host
  cudaMemcpy(out + (m / 2) * k * id, d_out, (m / 2) * k * sizeof(int),
...
void run_knn(workRequest *wr, cudaStream_t kernelStream, void **deviceBuffers) {
  ...
  cudaMalloc((void **)&D, (m / 2) * m * sizeof(int));
	...
	printf("entering run_knn with %d %d %d %d\n", m, n, k, tid);

  printf("running computeDist kernel\n");
...
void kernelSetup(void *cb, int tid, int m, int n, int k, int *h_A, int *&h_B) {
  ...
  h_B = (int *)info->hostBuffer;
	...
	// Setup the user data
	int *h_data = (int *)hapi_poolMalloc(4 * sizeof(int));
	int  vals[] = {m, n, k, tid};
	memcpy(h_data, vals, 4 * sizeof(int));

	wr->userData = h_data;

	printf("enqueuing workRequest\n");
